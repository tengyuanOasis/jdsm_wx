	...
	<view style="height: 100%">
		...
		<view class="container">
			...
			<dialog
				...
				:title="error3.title"
				:content="error3.msg"
				:cancelText="error3.cancel"
				:confirmText="error3.confirm"
			></dialog>
            ...
            <text class="order-other-3">回收价：</text>
							...
							<view class="order-button">
								...
								<view
									...
									:data-ordernumber="order.order_number"
									v-if="
										order.order_status == 'waitdelivery' ||
										(order.order_status == 'delivered' && order.expree_mode == 'visit')
									"
								>
									取消订单
								</view>
			...
			sendOut: function (e) {
				...
				if ((curTime.getTime() >= startTime.getTime()) & (curTime.getTime() <= endTime.getTime())) {
					uni.showModal({
						title: '放假提醒',
						content:
							'(2022.01.27~2022.02.07)春节放假，8日正常上班，订单超时不发货将过期，需重新估价下单，请合理安排发货时间）',
						confirmText: 'OK',
						showCancel: false,
						success(res) {
							if (res.confirm) {
