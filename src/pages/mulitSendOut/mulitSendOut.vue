    ...
    <text>全部待发货订单</text>
		...
		<view class="container">
			...
			<view class="order-box">
				<block
					v-for="(it, index) in allOrders"
					:key="index"
				>
					<view
						class="single-order"
						:data-number="it.order_number"
						@tap="choseFun"
					>
						<image
							class="main-img"
						...
						<view class="center">
							...
							<text class="text1 price"
								>回收价格：{{
									it.secondary_recycle == 0 ? it.recycle_money : it.secondary_recycle
								}}</text
							>
						</view>
						<view class="right">
							<image
								v-if="it.isChose"
								src="/static/images/hook2.png"
							></image>
							<image
								v-if="!it.isChose"
								src="/static/images/un_hook2.png"
							></image>
						</view>
					</view>
				</block>
			...
			<view
				class="save-btn"
				@tap="commit"
				>已选择{{ choseArray.length }}个订单，立即发货</view
			>
		</view>
		<!-- <kefu-cmp>
</kefu-cmp> -->
				...
				for (let i in datas) {
					...
					if (datas[i].isChose) {
						let temp = {
							...
							price:
								datas[i].secondary_recycle == 0
									? datas[i].recycle_money
									: datas[i].secondary_recycle
						};
