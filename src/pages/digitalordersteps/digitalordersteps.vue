		...
		<view class="container">
					...
					<view :id="Mitem[0].price" class="title-line">
						<view class="count-title-left">
							<image
								...
								style="width: 10rpx; height: 36rpx; margin-right: 20rpx"
							></image>
			...
			<view class="modalBox" v-if="showModal">
				...
				<image
					...
					mode="aspectFit"
					class="outImg"
					src="/static/pages/digitalordersteps/images/out.png"
				/>
	...
	export default {
		...
		methods: {
			...
			getMainData: function () {
				...
				//取出估价信息
				uni.request({
					url: `${domain.apiHost}/getAllVfoInfoForDigital`,
					header: {
						'content-type': 'application/x-www-form-urlencoded' // post请求需设置
					},
			...
			choseFun: function (event) {
				...
				for (let k = 0; k < temps[currentParentId].length; k++) {
					...
					if (
						currentPressId == temps[currentParentId][k][0] &&
						currentfeatureType == 'multiSelect'
					) {
							...
							if (
								...
								if (global_markup_money != 0) {
									...
									that.product_title.markup_money =
										global_markup_money + that.product_title.markup_money;
